...
public class Push : MonoBehaviour
{
    ...
    private void PushAction()
    {
        ...
        else if (Input.GetKeyUp(KeyCode.F))
        {
            CanPush = false;
            ObjectMoving = false;
            animator.SetBool("isPushing", false); // deactivating the push animation
        }

        if (ObjectMoving)
        {
            animator.SetBool("isPushing", true); // activating the push animation
        }
    }
    ...
    private void OnControllerColliderHit(ControllerColliderHit hit)
    {
        ...
        // no rigidbody
        if (Body == null || Body.tag != "Object") { return; } // check tag
